...
const logger = (0, logger_js_1.createLogger)('auth', 'oidc-flow');
// =============================================================================
...
function generateTOTPCode(secretEnvVar, env = process.env) {
    ...
    if (!secret) {
        ...
        throw new auth_error_js_1.ARTKAuthError(`TOTP secret environment variable "${secretEnvVar}" is not set`, 'unknown', 'mfa', undefined, `Set the ${secretEnvVar} environment variable with your TOTP secret`);
    }
    try {
        // Clean the secret (remove spaces, normalize case)
        const cleanSecret = secret.replace(/\s+/g, '').toUpperCase();
        // Generate TOTP code
        const code = otplib_1.authenticator.generate(cleanSecret);
        ...
        throw new auth_error_js_1.ARTKAuthError(`Failed to generate TOTP code: ${message}`, 'unknown', 'mfa', undefined, `Verify that ${secretEnvVar} contains a valid base32-encoded TOTP secret`);
    }
}
/**
...
function verifyTOTPCode(code, secretEnvVar, env = process.env) {
    ...
    try {
        const cleanSecret = secret.replace(/\s+/g, '').toUpperCase();
        return otplib_1.authenticator.verify({ token: code, secret: cleanSecret });
    }
    catch {
        return false;
    }
...
function getTimeUntilNextTOTPWindow() {
    const step = otplib_1.authenticator.options.step ?? 30;
...
async function executeOIDCFlow(page, config, credentials, options) {
    ...
    try {
        ...
        const duration = Date.now() - startTime;
        const authError = error instanceof auth_error_js_1.ARTKAuthError
            ? error
            : new auth_error_js_1.ARTKAuthError(error instanceof Error ? error.message : String(error), role, 'credentials');
...
async function navigateToLogin(page, config, role) {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new auth_error_js_1.ARTKAuthError(`Failed to navigate to login URL: ${message}`, role, 'navigation', undefined, `Verify the login URL is correct and accessible: ${config.loginUrl}`);
    }
}
/**
...
async function waitForIdPRedirect(page, config, role) {
    ...
    if (config.idpLoginUrl) {
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            throw new auth_error_js_1.ARTKAuthError(`Timeout waiting for IdP redirect: ${message}`, role, 'navigation', undefined, 'The application may not have redirected to the IdP login page');
        }
    }
    else {
...
async function fillIdPCredentials(page, config, credentials, idpHandler, role) {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new auth_error_js_1.ARTKAuthError(`Failed to fill credentials on IdP page: ${message}`, role, 'credentials', undefined, 'Check if the IdP selectors are correct for username/password fields');
    }
}
/**
...
async function submitIdPForm(page, config, idpHandler, role) {
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new auth_error_js_1.ARTKAuthError(`Failed to submit login form: ${message}`, role, 'credentials', undefined, 'Check if the submit button selector is correct');
    }
}
/**
...
async function handleMFA(page, mfaConfig, idpHandler, role) {
    ...
    switch (mfaConfig.type) {
        ...
        case 'push':
            ...
            break;
        case 'sms':
            throw new auth_error_js_1.ARTKAuthError('SMS-based MFA is not supported for automated testing', role, 'mfa', undefined, 'Configure TOTP-based MFA for the test account instead');
        case 'none':
...
async function handleTOTPMFA(page, mfaConfig, idpHandler, role) {
    if (!mfaConfig.totpSecretEnv) {
        throw new auth_error_js_1.ARTKAuthError('TOTP secret environment variable not configured', role, 'mfa', undefined, 'Configure mfa.totpSecretEnv in your artk.config.yml');
    }
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new auth_error_js_1.ARTKAuthError(`Failed to complete TOTP MFA: ${message}`, role, 'mfa', undefined, 'Check TOTP input selector configuration and verify the secret is correct');
    }
}
/**
...
async function handlePushMFA(page, mfaConfig, role) {
    ...
    try {
        ...
        throw new auth_error_js_1.ARTKAuthError(`Push MFA approval timeout after ${timeout}ms`, role, 'mfa', undefined, 'Approve the push notification on your device or configure TOTP instead');
    }
}
/**
...
async function waitForSuccess(page, config, role) {
    ...
    try {
        ...
        throw new auth_error_js_1.ARTKAuthError('Authentication callback failed', role, 'callback', errorText, 'Verify credentials are correct and the success URL/selector configuration');
    }
}
/**
