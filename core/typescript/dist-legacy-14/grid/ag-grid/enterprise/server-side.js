...
exports.SERVER_SIDE_SELECTORS = void 0;
exports.waitForBlockLoad = waitForBlockLoad;
exports.getServerSideState = getServerSideState;
exports.refreshServerSideData = refreshServerSideData;
exports.scrollToServerSideRow = scrollToServerSideRow;
exports.isRowLoaded = isRowLoaded;
exports.waitForInfiniteScrollLoad = waitForInfiniteScrollLoad;
 ...
 */
async function waitForBlockLoad(gridLocator, rowIndex, _config, options) {
    ...
    const viewport = gridLocator.locator(exports.SERVER_SIDE_SELECTORS.INFINITE_VIEWPORT);
    ...
    const loadingIndicator = gridLocator.locator(`${exports.SERVER_SIDE_SELECTORS.LOADING_CELL}, ${exports.SERVER_SIDE_SELECTORS.SKELETON_ROW}`);
    ...
    await (0, test_1.expect)(loadingIndicator).toHaveCount(0, { timeout });
    // Verify the target row is now available
    const targetRow = gridLocator.locator(`${selectors_js_1.AG_GRID_SELECTORS.ROW}[aria-rowindex="${rowIndex + 1}"]`);
    await (0, test_1.expect)(targetRow).toBeVisible({ timeout: 5000 });
}
/**
 ...
 */
async function getServerSideState(gridLocator, config) {
    // Check if currently loading
    const loadingCells = gridLocator.locator(exports.SERVER_SIDE_SELECTORS.LOADING_CELL);
    const skeletonRows = gridLocator.locator(exports.SERVER_SIDE_SELECTORS.SKELETON_ROW);
 ...
 * @param options - Server-side options
 */
async function refreshServerSideData(gridLocator, _config, options) {
    const timeout = options?.timeout ?? SERVER_SIDE_DEFAULTS.blockLoadTimeout;
    // Try to find and click a refresh button if available
    const refreshButton = gridLocator.locator('[data-action="refresh"], .ag-tool-panel-button[title*="Refresh"], button:has-text("Refresh")');
    ...
    // Wait for loading to complete
    await (0, test_1.expect)(loadingOverlay).not.toBeVisible({ timeout });
    await (0, test_1.expect)(loadingCells).toHaveCount(0, { timeout });
}
/**
 ...
 * @param options - Server-side options
 */
async function scrollToServerSideRow(gridLocator, rowIndex, config, options) {
    const timeout = options?.timeout ?? SERVER_SIDE_DEFAULTS.blockLoadTimeout;
    const viewport = gridLocator.locator(exports.SERVER_SIDE_SELECTORS.INFINITE_VIEWPORT);
    // Estimate row height
    const rowHeight = await estimateRowHeight(gridLocator);
 ...
 */
async function isRowLoaded(gridLocator, rowIndex) {
    // aria-rowindex is 1-based
    const ariaRowIndex = rowIndex + 1;
    // Check for row with data (not loading)
    const loadedRow = gridLocator.locator(`${selectors_js_1.AG_GRID_SELECTORS.ROW}[aria-rowindex="${ariaRowIndex}"]:not(.ag-loading):not(.ag-skeleton-row)`);
    const count = await loadedRow.count();
    if (count === 0) {
        return false;
    }
    // Verify it has actual cell content (not just a placeholder)
    const cells = loadedRow.locator(selectors_js_1.AG_GRID_SELECTORS.CELL);
    ...
    const hasLoading = await firstCell.locator(exports.SERVER_SIDE_SELECTORS.LOADING_CELL).count();
    return hasLoading === 0;
}
/**
 ...
 * @param options - Server-side options
 */
async function waitForInfiniteScrollLoad(gridLocator, expectedMinRows, _config, options) {
    const timeout = options?.timeout ?? SERVER_SIDE_DEFAULTS.blockLoadTimeout;
    await (0, test_1.expect)(async () => {
        const loadedRows = gridLocator.locator(exports.SERVER_SIDE_SELECTORS.LOADED_ROW);
        const count = await loadedRows.count();
        (0, test_1.expect)(count).toBeGreaterThanOrEqual(expectedMinRows);
    }).toPass({ timeout });
}
/**
 * Get the range of currently loaded row indices
 */
async function getLoadedRowRange(gridLocator) {
    const loadedRows = gridLocator.locator(exports.SERVER_SIDE_SELECTORS.LOADED_ROW);
    const count = await loadedRows.count();
    if (count === 0) {
        return null;
 ...
 */
async function getTotalServerRowCount(gridLocator) {
    // Try pagination panel first
    const paginationPanel = gridLocator.locator(exports.SERVER_SIDE_SELECTORS.PAGINATION_PANEL);
    ...
    // Try status bar
    const statusBar = gridLocator.locator(exports.SERVER_SIDE_SELECTORS.STATUS_BAR);
 ...
 */
async function estimateCachedBlocks(gridLocator, _config, blockSize = SERVER_SIDE_DEFAULTS.blockSize) {
 ...
 */
async function estimateRowHeight(gridLocator) {
 ...
 */
async function waitForLoadingComplete(gridLocator, timeout) {
    const loadingIndicators = gridLocator.locator(`${exports.SERVER_SIDE_SELECTORS.LOADING_CELL}, ${exports.SERVER_SIDE_SELECTORS.SKELETON_ROW}`);
    try {
        await (0, test_1.expect)(loadingIndicators).toHaveCount(0, { timeout });
    }
    catch {
