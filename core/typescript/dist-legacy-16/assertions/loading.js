...
async function expectLoading(page, options = {}, config) {
    ...
    const loadingSelectors = selectors ??
    ...
    await (0, test_1.expect)(loadingIndicator.first()).toBeVisible({ timeout });
}
/**
...
async function expectNotLoading(page, options = {}, config) {
    ...
    for (const selector of loadingSelectors) {
        ...
        await (0, test_1.expect)(loadingIndicator).not.toBeVisible({ timeout });
    }
}
/**
...
async function waitForLoadingComplete(page, options = {}, config) {
    ...
    const loadingIndicator = page.locator(combinedSelector);
    try {
        await (0, test_1.expect)(loadingIndicator.first()).toBeVisible({ timeout: 1000 });
    }
    catch {
    ...
    for (const selector of loadingSelectors) {
        ...
        await (0, test_1.expect)(indicator).not.toBeVisible({ timeout });
    }
}
/**
...
async function waitForLoadingOperation(page, operation, options = {}, config) {
    ...
    await (0, test_1.expect)(loadingIndicator.first()).toBeVisible({ timeout: appearTimeout });
    // Wait for loading to complete
