...
const catalogSchema_js_1 = require("./catalogSchema.js");
/**
...
function extractComponentName(filePath) {
    const baseName = (0, node_path_1.basename)(filePath, (0, node_path_1.extname)(filePath));
...
async function scanForTestIds(options) {
    const { sourceDir, testIdAttribute = 'data-testid', include = DEFAULT_INCLUDE, exclude = DEFAULT_EXCLUDE, trackCSSDebt = true, existingCatalog, } = options;
    const resolvedDir = (0, node_path_1.resolve)(sourceDir);
    const warnings = [];
    if (!(0, node_fs_1.existsSync)(resolvedDir)) {
        return {
            catalog: existingCatalog ?? (0, catalogSchema_js_1.createEmptyCatalog)(),
            filesScanned: 0,
            testIdsFound: 0,
            cssDebtFound: 0,
    ...
    // Find files to scan
    const files = await (0, fast_glob_1.default)(include, {
        cwd: resolvedDir,
        ignore: exclude,
        absolute: true,
    });
    // Start with existing or empty catalog
    const catalog = existingCatalog ?? (0, catalogSchema_js_1.createEmptyCatalog)();
    ...
    for (const filePath of files) {
        try {
            ...
            for (const selector of result.selectors) {
                ...
                selector.sourceFile = (0, node_path_1.relative)(resolvedDir, filePath);
            ...
            for (const debt of result.cssDebt) {
                ...
                debt.usages = debt.usages.map((u) => ({
                    ...u,
                    file: (0, node_path_1.relative)(resolvedDir, u.file),
                }));
