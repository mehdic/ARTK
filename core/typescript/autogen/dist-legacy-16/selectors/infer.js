...
function inferSelectors(text) {
    ...
    if (role && priority_js_1.NAMEABLE_ROLES.includes(role)) {
        if (name) {
            alternatives.push({
                strategy: 'role',
                value: role,
...
function inferBestSelector(text) {
    ...
    return (0, priority_js_1.selectBestLocator)(alternatives);
}
/**
...
function inferSelectorWithCatalog(text, options) {
    ...
    if (useCatalog) {
        const catalogEntry = (0, catalog_js_1.suggestSelector)(text);
        if (catalogEntry) {
            return {
        ...
        if (name) {
            ...
            for (const testId of possibleTestIds) {
                if ((0, catalog_js_1.hasTestId)(testId)) {
                    return {
                        strategy: 'testid',
                        value: testId,
                    };
