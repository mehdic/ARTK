...
const journey_js_1 = require("./journey.js");
...
async function validateCode(code, journey, frontmatter, options = {}) {
    ...
    if (opts.validateFrontmatter && frontmatter) {
        const frontmatterResult = (0, journey_js_1.validateJourneyFrontmatter)(frontmatter, {
    ...
    const patternResults = (0, patterns_js_1.scanForbiddenPatterns)(code);
    const patternIssues = (0, patterns_js_1.scanResultsToIssues)(patternResults);
    allIssues.push(...patternIssues);
    const patternSummary = (0, patterns_js_1.getViolationSummary)(patternResults);
    details.patterns = {
    ...
    // 3. ESLint (optional)
    if (opts.runLint) {
        const lintResult = await (0, lint_js_1.lintCode)(code, `${journey.id.toLowerCase()}.spec.ts`);
        details.lint = lintResult;
        allIssues.push(...lintResult.issues);
    }
    // 4. Tag validation
    if (opts.validateTags) {
        const tagResult = (0, tags_js_1.validateTagsInCode)(code, journey.id, journey.tier, journey.scope);
        details.tags = tagResult;
        allIssues.push(...tagResult.issues);
    }
    // 5. Coverage validation
    if (opts.validateCoverage) {
        const coverageResult = (0, coverage_js_1.validateIRCoverage)(journey, {
