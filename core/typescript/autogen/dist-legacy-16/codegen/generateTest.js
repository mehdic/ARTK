...
const updater_js_1 = require("../journey/updater.js");
    ...
    switch (primitive.type) {
        ...
        case 'waitForURL':
            const urlPattern = typeof primitive.pattern === 'string'
                ? `/${(0, escaping_js_1.escapeRegex)(primitive.pattern)}/`
        ...
        // Interactions
        case 'click':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.click();`;
        case 'fill':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.fill(${renderValue(primitive.value)});`;
        case 'select':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.selectOption('${escapeString(primitive.option)}');`;
        case 'check':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.check();`;
        case 'uncheck':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.uncheck();`;
        case 'press':
            if (primitive.locator) {
                return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.press('${primitive.key}');`;
            }
            return `${indent}await page.keyboard.press('${primitive.key}');`;
        case 'hover':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.hover();`;
        case 'focus':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.focus();`;
        case 'clear':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.clear();`;
        case 'upload':
            return `${indent}await page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}.setInputFiles([${primitive.files.map(f => `'${escapeString(f)}'`).join(', ')}]);`;
        // Assertions
        case 'expectVisible':
            const visibleOptions = primitive.timeout ? `{ timeout: ${primitive.timeout} }` : '';
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toBeVisible(${visibleOptions});`;
        case 'expectNotVisible':
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).not.toBeVisible();`;
        case 'expectHidden':
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toBeHidden();`;
        case 'expectText':
            const textPattern = typeof primitive.text === 'string'
                ? `'${escapeString(primitive.text)}'`
                : primitive.text.toString();
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toHaveText(${textPattern});`;
        case 'expectValue':
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toHaveValue('${escapeString(primitive.value)}');`;
        case 'expectChecked':
            if (primitive.checked === false) {
                return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).not.toBeChecked();`;
            }
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toBeChecked();`;
        case 'expectEnabled':
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toBeEnabled();`;
        case 'expectDisabled':
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toBeDisabled();`;
        case 'expectURL':
        ...
        case 'expectCount':
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toHaveCount(${primitive.count});`;
        case 'expectContainsText':
            return `${indent}await expect(page.${(0, priority_js_1.toPlaywrightLocator)(primitive.locator)}).toContainText('${escapeString(primitive.text)}');`;
        // Signals
        case 'expectToast':
...
function loadDefaultTemplate() {
    const templatePath = (0, paths_js_1.getTemplatePath)('test.ejs');
...
function generateTest(journey, options = {}) {
    ...
    let code = ejs_1.default.render(template, {
        journey,
        imports,
        renderPrimitive,
        escapeString,
        escapeRegex: escaping_js_1.escapeRegex,
    ...
    if (strategy === 'blocks' && existingCode) {
        ...
        code = (0, blocks_js_1.injectManagedBlocks)({
            existingCode,
            newBlocks: [testBlock],
        });
    }
    ...
    if (options.updateJourney && options.journeyPath) {
        try {
            ...
            (0, updater_js_1.updateJourneyFrontmatter)({
            ...
            console.error(`Warning: Failed to update journey frontmatter: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return {
        code,
