 ...
 */
export function extractCSSSelector(code) {
 ...
 */
export function containsCSSSelector(code) {
 ...
 */
export function inferRoleFromSelector(selector) {
 ...
 */
export function extractNameFromSelector(selector) {
 ...
 */
export function generateRoleLocator(role, name, options) {
    const parts = [];
    if (name) {
        if (options?.exact) {
 ...
 */
export function generateLabelLocator(label, exact) {
    if (exact) {
 ...
 */
export function generateTextLocator(text, exact) {
    if (exact) {
 ...
 */
export function generateTestIdLocator(testId) {
    return `page.getByTestId('${testId}')`;
}
/**
 * Apply selector fix to code
 */
export function applySelectorFix(context) {
 ...
 */
export function addExactToLocator(code) {
    let modifiedCode = code;
    let applied = false;
    // Add exact: true to getByRole
    modifiedCode = modifiedCode.replace(/page\.getByRole\s*\(\s*['"](\w+)['"]\s*,\s*\{\s*name:\s*['"]([^'"]+)['"]\s*\}\s*\)/g, (_, role, name) => {
        applied = true;
        return `page.getByRole('${role}', { name: '${name}', exact: true })`;
    });
    // Add exact: true to getByLabel
    modifiedCode = modifiedCode.replace(/page\.getByLabel\s*\(\s*['"]([^'"]+)['"]\s*\)/g, (_, label) => {
        applied = true;
        return `page.getByLabel('${label}', { exact: true })`;
    });
    // Add exact: true to getByText
    modifiedCode = modifiedCode.replace(/page\.getByText\s*\(\s*['"]([^'"]+)['"]\s*\)/g, (_, text) => {
        applied = true;
        return `page.getByText('${text}', { exact: true })`;
    });
    return {
