 ...
 */
export function recordCSSDebt(selector, file, line, reason) {
 ...
 */
export function suggestReplacement(selector) {
 ...
 */
export function generateDebtReport(catalog) {
 ...
 */
export function generateMigrationPlan(catalog) {
    const cat = catalog ?? getCatalog();
    const debt = cat.cssDebt ?? [];
 ...
 */
export function removeDebt(selector) {
    const catalog = getCatalog();
 ...
 */
export function updateDebtPriority(selector, priority) {
    const catalog = getCatalog();
 ...
 */
export function generateDebtMarkdown(catalog) {
    ...
    for (const entry of plan.entries.slice(0, 15)) {
        lines.push(`| \`${entry.debt.selector}\` | ${entry.debt.usages.length} | ${entry.debt.priority} | \`${entry.suggestedFix}\` |`);
    }
