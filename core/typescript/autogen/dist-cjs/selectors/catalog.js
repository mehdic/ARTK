...
function loadCatalog(path) {
    const resolvedPath = (0, node_path_1.resolve)(path ?? DEFAULT_CATALOG_PATH);
    if (!(0, node_fs_1.existsSync)(resolvedPath)) {
        console.warn(`Selector catalog not found at ${resolvedPath}, using empty catalog`);
        return (0, catalogSchema_js_1.createEmptyCatalog)();
    }
    try {
        const content = (0, node_fs_1.readFileSync)(resolvedPath, 'utf-8');
        const parsed = JSON.parse(content);
        const result = (0, catalogSchema_js_1.validateCatalog)(parsed);
        if (!result.valid) {
...
function saveCatalog(catalog, path) {
    const resolvedPath = (0, node_path_1.resolve)(path ?? catalogPath ?? DEFAULT_CATALOG_PATH);
    // Ensure directory exists
    const dir = (0, node_path_1.dirname)(resolvedPath);
    if (!(0, node_fs_1.existsSync)(dir)) {
        (0, node_fs_2.mkdirSync)(dir, { recursive: true });
    }
    // Update generation timestamp
    catalog.generatedAt = new Date().toISOString();
    // Calculate stats
    catalog.stats = calculateStats(catalog);
    (0, node_fs_1.writeFileSync)(resolvedPath, JSON.stringify(catalog, null, 2));
