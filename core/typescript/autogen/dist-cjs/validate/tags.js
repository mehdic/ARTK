...
function categorizeTags(tags) {
    ...
    for (const tag of tags) {
        if (exports.TAG_PATTERNS.journeyId.test(tag)) {
            result.journeyId = tag;
        }
        else if (exports.TAG_PATTERNS.tier.test(tag)) {
            result.tier = tag;
        }
        else if (exports.TAG_PATTERNS.scope.test(tag)) {
            result.scope = tag;
        }
        else if (exports.TAG_PATTERNS.actor.test(tag)) {
            result.actor = tag;
        }
        else if (exports.TAG_PATTERNS.custom.test(tag)) {
...
function validateTags(tags, journeyId, tier, scope, options = {}) {
    ...
    for (const tag of tags) {
        if (!exports.TAG_PATTERNS.custom.test(tag)) {
            issues.push({
