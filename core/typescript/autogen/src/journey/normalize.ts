 ...
 */
function parseStepText(text: string, warnings: string[]): IRPrimitive | null {
  ...
  if (lowerText.includes('enter') || lowerText.includes('type') || lowerText.includes('fill')) {
    const fillMatch = text.match(
      /(?:enter|type|fill)\s+["']([^"']+)["']\s+(?:in|into)\s+(?:the\s+)?["']?([^"']+)["']?\s*(?:field|input)?/i
    );
    if (fillMatch) {
      return {
        type: 'fill',
    ...
    const fillMatch2 = text.match(
      /(?:enter|type|fill)\s+(?:the\s+)?["']?([^"']+)["']?\s+(?:field|input)\s+(?:with\s+)?["']([^"']+)["']/i
    );
    if (fillMatch2) {
      return {
        type: 'fill',
  ...
  if (lowerText.includes('select')) {
    const selectMatch = text.match(
      /select\s+["']([^"']+)["']\s+(?:from|in)\s+(?:the\s+)?["']?([^"']+)["']?\s*(?:dropdown|select)?/i
    );
    if (selectMatch) {
      return {
        type: 'select',
  ...
  if (lowerText.includes('see') || lowerText.includes('visible') || lowerText.includes('displayed')) {
    const seeMatch = text.match(
      /(?:should\s+)?(?:see|visible|displayed)\s+(?:the\s+)?["']([^"']+)["']/i
    );
    if (seeMatch) {
      return {
        type: 'expectVisible',
  ...
  if (lowerText.includes('shows') || lowerText.includes('displays') || lowerText.includes('contains')) {
    const textMatch = text.match(
      /(?:shows?|displays?|contains?)\s+(?:the\s+)?(?:text\s+)?["']([^"']+)["']/i
    );
    if (textMatch) {
      return {
        type: 'expectText',
 ...
 * Create a value spec
 */
function createValue(value: string): ValueSpec {
