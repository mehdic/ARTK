...
var PackageScanner = class {
   ...
   */
  async scan(dirPath) {
    const packageJsonPath = path5__default.join(dirPath, "package.json");
    const emptyResult = {
      found: false,
...
function hasPackageJson(dirPath) {
  return existsSync(path5__default.join(dirPath, "package.json"));
}
  ...
  async detect(dirPath) {
    ...
    for (const [category, patterns] of Object.entries(ENTRY_FILE_PATTERNS)) {
      for (const pattern of patterns) {
        const fullPath = path5__default.join(dirPath, pattern);
...
var DirectoryAnalyzer = class {
   ...
   */
  analyze(dirPath) {
    const dirName = path5__default.basename(dirPath).toLowerCase();
    const signals = [];
    const detailedSignals = [];
  ...
  async scanRecursive(currentPath, rootPath, depth, maxDepth, results) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const entryPath = path5__default.join(currentPath, entry.name);
...
var FrontendDetector = class {
  ...
  async scanDirectory(currentPath, depth, options, results, visited) {
    ...
    const normalizedPath = path5__default.resolve(currentPath);
  ...
  async scanSubdirectories(currentPath, depth, options, results, visited) {
    try {
      ...
      for (const entry of entries) {
        ...
        const subPath = path5__default.join(currentPath, entry.name);
  ...
  async analyzeDirectory(dirPath, relativeTo) {
    ...
    const relativePath = path5__default.relative(relativeTo, dirPath);
    return {
      path: path5__default.resolve(dirPath),
  ...
  async checkIndexHtml(dirPath) {
    ...
    for (const location of locations) {
      const fullPath = path5__default.join(dirPath, location);
...
function generateTargetName(result, index) {
  const dirName = path5__default.basename(result.path).toLowerCase();
...
var SubmoduleChecker = class {
  ...
  async checkAll(repoRoot, options) {
    ...
    const gitmodulesPath = path5__default.join(repoRoot, ".gitmodules");
    ...
    const statuses = [];
    for (const entry of entries) {
      const fullPath = path5__default.join(repoRoot, entry.path);
      const status = {
        path: entry.path,
        initialized: false
      };
      ...
      if (opts.checkInitStatus) {
        ...
        status.initialized = existsSync(path5__default.join(fullPath, ".git"));
      }
  ...
  async scan(dirPath, repoRoot) {
    ...
    const relativePath = path5__default.relative(repoRoot, dirPath);
    const entries = await this.parseGitmodules(gitmodulesPath);
  ...
  async isInSubmodule(checkPath, repoRoot) {
    ...
    const relativePath = path5__default.relative(repoRoot, checkPath);
...
function detectModuleSystem(projectRoot) {
  const warnings = [];
  const packageJsonPath = path5.join(projectRoot, "package.json");
...
function readCachedContext(projectRoot) {
  const contextPath = path5.join(projectRoot, CONTEXT_DIR, CONTEXT_FILE);
  if (!fs3.existsSync(contextPath)) {
    return null;
  }
  try {
    const content = fs3.readFileSync(contextPath, "utf8");
    return JSON.parse(content);
  } catch {
    return null;
  }
...
function writeCachedContext(projectRoot, context) {
  ...
  const contextPath = path5.join(contextDir, CONTEXT_FILE);
  if (!fs3.existsSync(contextDir)) {
    fs3.mkdirSync(contextDir, { recursive: true });
  }
  fs3.writeFileSync(contextPath, JSON.stringify(context, null, 2));
}
...
export { AG_GRID_PACKAGE_INDICATORS, CONFIDENCE_THRESHOLDS, DIRECTORY_PATTERNS, DirectoryAnalyzer, ENTRY_FILE_PATTERNS, EntryFileDetector, FRAMEWORK_DETECTION_MAP, FRONTEND_DIRECTORY_PATTERNS, FRONTEND_PACKAGE_INDICATORS, FrontendDetector, NON_FRONTEND_PATTERNS, PackageScanner, SIGNAL_WEIGHTS, SubmoduleChecker, analyzeDirectoryName, calculateConfidence, calculateScore, checkSubmodules, clearWarnedSignalsCache, createSignal, detectEntryFiles, detectEnvironment, detectFrontends, detectModuleSystem, detectSingleFrontend, detectTypeScriptModule, detectionResultsToTargets, determineESMCompatibility, filterByConfidence, getAllEntryPatterns, getConfidenceFromScore, getModuleSystemFromType, getNodeVersion, getSignalCategory, getSignalWeight, getTsModuleFromConfig, hasEntryFiles, hasPackageJson, inferModuleSystemFromTsModule, isAgGridEnterprisePackage, isAgGridPackage, isFrontendDirectory, isFrontendPackage, isNonFrontendDirectory, isPathInSubmodule, isVersionGte, matchesFrontendDirectoryPattern, parseGitmodulesFile, parseNodeVersion, parsePackageJson, parseTsConfig, scanForFrontendDirectories, scanPackageJson, scanSubmodule, validateNodeVersion };
