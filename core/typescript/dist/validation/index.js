...
function createBackupDir() {
  ...
  const backupDir = path2.join(tempDir, ".artk-backup-" + Date.now());
...
function trackOriginalFile(tx, filePath) {
  ...
  const backupPath = path2.join(backupDir, fileName + "." + Date.now() + ".bak");
  try {
...
function rollbackTransaction(tx) {
  ...
  for (const [originalPath, backupPath] of tx.originalFiles) {
    try {
      if (fs2.existsSync(backupPath)) {
        const dir = path2.dirname(originalPath);
        if (!fs2.existsSync(dir)) {
          fs2.mkdirSync(dir, { recursive: true });
        }
        fs2.copyFileSync(backupPath, originalPath);
...
var ValidationRunner = class {
  ...
  async persistResult(result) {
    const artkDir = path2.join(this.outputDir, ".artk");
    const resultsPath = path2.join(artkDir, "validation-results.json");
    if (!fs2.existsSync(artkDir)) {
      fs2.mkdirSync(artkDir, { recursive: true });
    }
    let results = [];
    if (fs2.existsSync(resultsPath)) {
      try {
        const content = fs2.readFileSync(resultsPath, "utf8");
    ...
    results.push(result);
    fs2.writeFileSync(resultsPath, JSON.stringify(results, null, 2));
  }
};
