 ...
 */
export const DEFAULT_HEALING_RULES = [
    {
 ...
 */
export const DEFAULT_HEALING_CONFIG = {
    enabled: true,
    maxAttempts: 3,
    allowedFixes: [
        'selector-refine',
 ...
 */
export const UNHEALABLE_CATEGORIES = [
 ...
 */
export function isCategoryHealable(category) {
    return !UNHEALABLE_CATEGORIES.includes(category);
}
/**
 * Get applicable healing rules for a failure classification
 */
export function getApplicableRules(classification, config = DEFAULT_HEALING_CONFIG) {
    if (!config.enabled) {
        return [];
    }
 ...
 */
export function evaluateHealing(classification, config = DEFAULT_HEALING_CONFIG) {
    if (!config.enabled) {
        return {
            canHeal: false,
            applicableFixes: [],
 ...
 */
export function getNextFix(classification, attemptedFixes, config = DEFAULT_HEALING_CONFIG) {
 ...
 */
export function isFixAllowed(fixType, config = DEFAULT_HEALING_CONFIG) {
    return config.enabled && config.allowedFixes.includes(fixType);
}
/**
 * Validate that a fix is not forbidden
 */
export function isFixForbidden(fixType) {
    const forbidden = [
        'add-sleep',
        'remove-assertion',
        'weaken-assertion',
 ...
 */
export function getHealingRecommendation(classification) {
 ...
 */
export function getPostHealingRecommendation(classification, attemptCount) {
