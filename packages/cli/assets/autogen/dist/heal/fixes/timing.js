 ...
 */
export function extractTimeoutFromError(errorMessage) {
 ...
 */
export function suggestTimeoutIncrease(currentTimeout, maxTimeout = 30000) {
 ...
 */
export function fixMissingAwait(code) {
 ...
 */
export function convertToWebFirstAssertion(code) {
    let modifiedCode = code;
    let applied = false;
    // Convert textContent extraction to toHaveText
    modifiedCode = modifiedCode.replace(/const\s+(\w+)\s*=\s*await\s+(\w+)\.textContent\s*\(\s*\)\s*;?\s*\n(\s*)expect\s*\(\s*\1\s*\)\.toBe\s*\(\s*(['"][^'"]+['"])\s*\)/g, (_, _varName, locator, indent, expected) => {
        applied = true;
        return `${indent}await expect(${locator}).toHaveText(${expected})`;
    });
    // Convert innerText extraction to toHaveText
    modifiedCode = modifiedCode.replace(/const\s+(\w+)\s*=\s*await\s+(\w+)\.innerText\s*\(\s*\)\s*;?\s*\n(\s*)expect\s*\(\s*\1\s*\)\.toBe\s*\(\s*(['"][^'"]+['"])\s*\)/g, (_, _varName, locator, indent, expected) => {
        applied = true;
        return `${indent}await expect(${locator}).toHaveText(${expected})`;
    });
    // Convert isVisible check to toBeVisible
    modifiedCode = modifiedCode.replace(/const\s+(\w+)\s*=\s*await\s+(\w+)\.isVisible\s*\(\s*\)\s*;?\s*\n(\s*)expect\s*\(\s*\1\s*\)\.toBe\s*\(\s*true\s*\)/g, (_, _varName, locator, indent) => {
        applied = true;
        return `${indent}await expect(${locator}).toBeVisible()`;
    });
    // Convert isHidden check to toBeHidden
    modifiedCode = modifiedCode.replace(/const\s+(\w+)\s*=\s*await\s+(\w+)\.isHidden\s*\(\s*\)\s*;?\s*\n(\s*)expect\s*\(\s*\1\s*\)\.toBe\s*\(\s*true\s*\)/g, (_, _varName, locator, indent) => {
        applied = true;
 ...
 */
export function addTimeout(code, lineNumber, timeout) {
    ...
    modifiedLine = modifiedLine.replace(/\.(click|fill|press|type|hover|focus|check|uncheck)\s*\(\s*\)/g, `.$1({ timeout: ${timeout} })`);
    // Pattern: .click('text'), .fill('selector', 'value')
    modifiedLine = modifiedLine.replace(/\.(click|fill|press|type|hover|focus|check|uncheck)\s*\(\s*(['"][^'"]*['"])\s*\)/g, `.$1($2, { timeout: ${timeout} })`);
    // Pattern: .click({ options })
    modifiedLine = modifiedLine.replace(/\.(click|fill|press|type|hover|focus|check|uncheck)\s*\(\s*\{([^}]*)\}\s*\)/g, (_, action, options) => {
    ...
    modifiedLine = modifiedLine.replace(/\.(toBeVisible|toBeHidden|toHaveText|toContainText|toHaveValue)\s*\(\s*\)/g, `.$1({ timeout: ${timeout} })`);
 ...
 */
export function applyTimingFix(context) {
 ...
 */
export function wrapWithExpectToPass(code, lineStart, lineEnd, options) {
    const lines = code.split('\n');
 ...
 */
export function wrapWithExpectPoll(_code, _lineNumber, getter, expected, options) {
    const optParts = [];
    if (options?.timeout) {
        optParts.push(`timeout: ${options.timeout}`);
    }
