...
import { DEFAULT_HEALING_CONFIG, evaluateHealing, getNextFix, getPostHealingRecommendation, } from './rules.js';
import { HealingLogger } from './logger.js';
 ...
 */
export async function runHealingLoop(options) {
    const { journeyId, testFile, outputDir, config = DEFAULT_HEALING_CONFIG, verifyFn, ariaInfo, } = options;
    // Initialize logger
 ...
 */
export function previewHealingFixes(code, classification, config = DEFAULT_HEALING_CONFIG) {
    const previews = [];
    const evaluation = evaluateHealing(classification, config);
    if (!evaluation.canHeal) {
        return previews;
 ...
 */
export function wouldFixApply(code, fixType, classification) {
    const result = applyFix(code, fixType, {
        lineNumber: 1,
        errorMessage: '',
        classification,
