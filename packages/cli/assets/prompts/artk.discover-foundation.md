...
├── app-profile.json            # Application DNA (manually authored)
├── discovered-profile.json     # Auto-generated by F12 discovery
...
### 11.2 Generate discovered-profile.json from Discovery Data
...
✓ Generated discovered-profile.json from discovery data
...
**Execute AFTER F11 (LLKB initialization). Generates app-specific patterns from source code analysis.**
...
- 16 form operations × N forms (+ per-field patterns)
...
|-----------------|---------|-----------------|
| **Strong** (0.85) | Discovery patterns (routes, schemas, auth) | Raised to 0.85 minimum |
...
| Scenario | Behavior |
|----------|----------|
| No package.json | Log warning, skip framework detection, continue with source mining |
| Mining finds no entities | Log warning, use seed patterns only |
| Pattern generation fails | Log error with cause, keep seed patterns, continue |
| Discovery timeout (>30s) | Abort discovery, log warning, continue with seed patterns |
