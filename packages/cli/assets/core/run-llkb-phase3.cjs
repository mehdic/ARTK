 ...
 *   F12: Pattern discovery (priority order):
 *        1. Vendored pipeline: import vendor/artk-core/dist/llkb/index.js → runFullDiscoveryPipeline()
 *        2. CLI fallback: npx artk-autogen llkb-patterns discover (if installed)
 *        3. Soft fail: continue with seed patterns only
...
/**
 * Run the discovery pipeline directly from the vendored @artk/core.
 *
 * The vendored core is ESM but this file is CJS. To bridge:
 * 1. Write a temp .mjs script that dynamic-imports the vendored pipeline
 * 2. Execute it via execFileSync(process.execPath, ...) — no shell, no injection
 * 3. Clean up the temp script in a finally block
 *
 * Returns { success: true/false, discoveredCount?: number }
 */
function runDiscoveryFromVendor(projectRoot, llkbRoot, harnessRoot, opts) {
  var verbose = opts.verbose;
  var vendorPipelinePath = path.join(harnessRoot, 'vendor', 'artk-core', 'dist', 'llkb', 'index.js');

  if (!fs.existsSync(vendorPipelinePath)) {
    if (verbose) {
      console.log('  Vendored pipeline not found: ' + vendorPipelinePath);
    }
    return { success: false, reason: 'vendored pipeline not found' };
  }

  console.log('');
  console.log('--- [F12] Run LLKB Pattern Discovery (vendored pipeline) ---');
  if (verbose) {
    console.log('  Pipeline: ' + vendorPipelinePath);
    console.log('  Project root: ' + projectRoot);
    console.log('  LLKB root: ' + llkbRoot);
  }

  // Create temp directory for the wrapper script
  var tmpDir = path.join(llkbRoot, '.tmp');
  var tempScript = path.join(tmpDir, 'run-discovery.mjs');

  // The temp .mjs wrapper script content
  // Uses pathToFileURL to handle Windows backslash paths correctly
  var scriptContent = [
    '// Temp script generated by run-llkb-phase3.cjs — safe to delete',
    'import { pathToFileURL } from "node:url";',
    '',
    'const pipelinePath = process.argv[2];',
    'const projectRoot = process.argv[3];',
    'const llkbRoot = process.argv[4];',
    '',
    'async function main() {',
    '  const mod = await import(pathToFileURL(pipelinePath).href);',
    '  const result = await mod.runFullDiscoveryPipeline(projectRoot, llkbRoot);',
    '',
    '  // Print results for parent process to parse',
    '  console.log("PIPELINE_SUCCESS=" + result.success);',
    '  if (result.stats) {',
    '    console.log("PIPELINE_DURATION=" + result.stats.durationMs);',
    '    console.log("PIPELINE_TOTAL=" + (result.stats.totalGenerated || 0));',
    '    console.log("PIPELINE_AFTER_QC=" + (result.stats.afterQualityControl || 0));',
    '  }',
    '  if (result.patternsFile && result.patternsFile.patterns) {',
    '    console.log("PIPELINE_PATTERNS=" + result.patternsFile.patterns.length);',
    '  }',
    '  if (result.warnings && result.warnings.length > 0) {',
    '    result.warnings.forEach(function(w) { console.log("PIPELINE_WARN=" + w); });',
    '  }',
    '  if (result.errors && result.errors.length > 0) {',
    '    result.errors.forEach(function(e) { console.error("PIPELINE_ERR=" + e); });',
    '  }',
    '',
    ...
    '  console.error("PIPELINE_ERR=" + (err.message || err));',
    '  process.exit(1);',
    '});',
  ].join('\n');
  ...
  try {
    ...
    // Execute: node run-discovery.mjs <pipelinePath> <projectRoot> <llkbRoot>
    var stdout = execFileSync(process.execPath, [
      tempScript,
      vendorPipelinePath,
      projectRoot,
      llkbRoot,
      ...
      cwd: harnessRoot,
      env: Object.assign({}, process.env),
      timeout: opts.timeout || TIMEOUT_F12,
      encoding: 'utf-8',
    ...
    lines.forEach(function(line) {
      if (line.startsWith('PIPELINE_SUCCESS=')) {
        pipelineSuccess = line.slice('PIPELINE_SUCCESS='.length).trim() === 'true';
      } else if (line.startsWith('PIPELINE_PATTERNS=')) {
        patternCount = parseInt(line.slice('PIPELINE_PATTERNS='.length).trim(), 10) || 0;
      } else if (line.startsWith('PIPELINE_WARN=') && verbose) {
        console.log('  Pipeline warning: ' + line.slice('PIPELINE_WARN='.length));
      }
    });
    ...
    if (pipelineSuccess) {
      console.log('  Result: SUCCESS');
      if (patternCount >= 0) {
        console.log('  Discovered patterns: ' + patternCount);
      }
      return { success: true, discoveredCount: patternCount };
    } else {
      console.error('  Result: FAILED (pipeline returned success=false)');
      return { success: false, reason: 'pipeline returned success=false' };
    }
  } catch (err) {
    console.error('  Result: FAILED');
    if (verbose && err.message) {
    ...
    try {
      ...
      // Remove .tmp dir only if empty
      if (fs.existsSync(tmpDir)) {
        var remaining = fs.readdirSync(tmpDir);
...
function main() {
  ...
  if (args.skipDiscover) {
    ...
    // Priority 1: Try vendored pipeline (works even without artk-autogen in node_modules)
    f12 = runDiscoveryFromVendor(projectRoot, llkbRoot, harnessRoot, {
      verbose: args.verbose,
      timeout: TIMEOUT_F12,
    });

    // Priority 2: Fall back to npx artk-autogen if vendored pipeline failed
    if (!f12.success && isAutogenAvailable(harnessRoot)) {
      console.log('');
      console.log('  Vendored pipeline failed, falling back to artk-autogen CLI...');
      var f12Args = [
        'artk-autogen', 'llkb-patterns', 'discover',
        '--project-root', projectRoot,
        '--llkb-root', llkbRoot,
      ];
      f12 = runNpxCommand('[F12] Run LLKB Pattern Discovery Pipeline (CLI fallback)', f12Args, {
        verbose: args.verbose,
        cwd: harnessRoot,
        timeout: TIMEOUT_F12,
      });
