...
async function upgradeCommand(targetPath, options) {
  ...
  if (fs3.existsSync(vendorCorePath)) {
    try {
      const pkg = await fs3.readJson(vendorCorePath);
  ...
  try {
    ...
    const coreTarget = path3.join(artkE2ePath, "vendor", "artk-core");
    if (fs3.existsSync(coreSource)) {
      await fs3.remove(coreTarget);
      await fs3.copy(coreSource, coreTarget);
    } else {
    ...
    const autogenTarget = path3.join(artkE2ePath, "vendor", "artk-core-autogen");
    if (fs3.existsSync(autogenSource)) {
      await fs3.remove(autogenTarget);
      await fs3.copy(autogenSource, autogenTarget);
    }
...
async function checkArtkInstallation(projectPath, artkE2ePath) {
  ...
  if (!fs3.existsSync(contextPath)) {
    return {
      ...
      fix: async () => {
        await fs3.ensureDir(path3.join(projectPath, ".artk"));
        await fs3.writeJson(contextPath, {
          version: "1.0",
          projectRoot: projectPath,
          artkRoot: artkE2ePath,
          initialized_at: (/* @__PURE__ */ new Date()).toISOString()
...
async function checkNodeModules(artkE2ePath) {
  ...
  if (!fs3.existsSync(playwrightPath)) {
    return {
      name: "Dependencies",
      status: "error",
...
async function checkPlaywrightBrowsers(projectPath, artkE2ePath, logger2) {
  ...
  const hasCachedBrowsers = fs3.existsSync(browsersCachePath) && fs3.readdirSync(browsersCachePath).some((f) => f.startsWith("chromium-"));
...
async function checkConfigFiles(artkE2ePath) {
  ...
  for (const file of requiredFiles) {
    if (!fs3.existsSync(path3.join(artkE2ePath, file))) {
      missing.push(file);
    }
  }
  if (missing.length > 0) {
...
async function checkVendorPackages(artkE2ePath) {
  ...
  const issues = [];
  if (!fs3.existsSync(path3.join(vendorCore, "package.json"))) {
    issues.push("@artk/core");
  }
  if (!fs3.existsSync(path3.join(vendorAutogen, "package.json"))) {
...
async function checkTypeScript(artkE2ePath) {
  if (!fs3.existsSync(path3.join(artkE2ePath, "node_modules"))) {
    return {
      name: "TypeScript",
      status: "warn",
...
async function uninstallCommand(targetPath, options) {
  ...
  if (!fs3.existsSync(artkE2ePath) && !fs3.existsSync(artkDir)) {
    logger2.error("ARTK is not installed in this project.");
    process.exit(1);
  }
  ...
  if (fs3.existsSync(artkDir)) {
    toRemove.push({
      path: artkDir,
      description: ".artk/ (ARTK metadata and browser cache)"
    });
  }
  if (!options.keepPrompts && fs3.existsSync(promptsDir)) {
    const artkPrompts = fs3.readdirSync(promptsDir).filter((f) => f.startsWith("artk.") && f.endsWith(".prompt.md"));
  ...
  try {
    for (const item of toRemove) {
      if (fs3.existsSync(item.path)) {
        await fs3.remove(item.path);
      }
    }
    if (fs3.existsSync(promptsDir)) {
      const remaining = fs3.readdirSync(promptsDir);
      if (remaining.length === 0) {
        await fs3.remove(promptsDir);
      }
    }
    const githubDir = path3.join(resolvedPath, ".github");
    if (fs3.existsSync(githubDir)) {
      const remaining = fs3.readdirSync(githubDir);
      if (remaining.length === 0) {
        await fs3.remove(githubDir);
      }
    }
