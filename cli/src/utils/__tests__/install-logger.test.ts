  ...
  describe('InstallLogger', () => {
    describe('info', () => {
      it('should write INFO log entry', () => {
        ...
        expect(fs.appendFileSync).toHaveBeenCalledWith(
          expect.any(String),
          expect.stringContaining('"operation":"install"'),
          expect.any(String)
        );
        expect(fs.appendFileSync).toHaveBeenCalledWith(
          expect.any(String),
      ...
      it('should include details when provided', () => {
        ...
        expect(fs.appendFileSync).toHaveBeenCalledWith(
          expect.any(String),
          expect.stringContaining('"details":{"key":"value"}'),
          expect.any(String)
        );
      });
    });
    ...
    describe('logInstallStart', () => {
      it('should log installation start with variant and node version', () => {
        ...
        expect(fs.appendFileSync).toHaveBeenCalledWith(
          expect.any(String),
          expect.stringContaining('Starting ARTK installation'),
          expect.any(String)
        );
        expect(fs.appendFileSync).toHaveBeenCalledWith(
          expect.any(String),
          expect.stringContaining('"variant":"modern-esm"'),
          expect.any(String)
        );
        expect(fs.appendFileSync).toHaveBeenCalledWith(
          expect.any(String),
          expect.stringContaining('"nodeVersion":20'),
          expect.any(String)
        );
      });
    });
    ...
    describe('logDetection', () => {
      it('should log detection results', () => {
        ...
        expect(fs.appendFileSync).toHaveBeenCalledWith(
          expect.any(String),
          expect.stringContaining('Environment detected'),
          expect.any(String)
        );
      });
    });
    ...
    describe('logUpgradeStart', () => {
      it('should log upgrade start', () => {
        ...
        expect(fs.appendFileSync).toHaveBeenCalledWith(
          expect.any(String),
          expect.stringContaining('Starting variant upgrade'),
          expect.any(String)
        );
      });
    });
    ...
    describe('readRecent', () => {
      ...
      it('should limit returned entries', () => {
        const logLines = Array(100)
          ...
          .map(
            (_, i) =>
              `{"timestamp":"2025-01-19T10:00:00.000Z","level":"INFO","operation":"install","message":"Entry ${i}"}`
          )
          .join('\n');
