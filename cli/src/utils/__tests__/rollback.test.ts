...
describe('rollback', () => {
  ...
  describe('rollback function', () => {
    it('should remove vendor directories that exist', () => {
      ...
      expect(fs.rmSync).toHaveBeenCalledWith(
        expect.stringContaining('artk-core'),
  ...
  describe('needsRollback', () => {
    ...
    it('should return true when context exists but vendor is missing', () => {
      vi.mocked(fs.existsSync).mockImplementation((p) => {
        const pathStr = String(p);
        if (pathStr.includes('context.json')) return true;
        if (pathStr.includes('vendor')) return false;
        return false;
      });
    ...
    it('should return true when vendor core is incomplete (missing index.js)', () => {
      vi.mocked(fs.existsSync).mockImplementation((p) => {
        const pathStr = String(p);
        // context.json exists
        if (pathStr.endsWith('context.json')) return true;
        // artk-core directory exists (but not autogen path check)
        if (pathStr.endsWith('artk-core')) return true;
        // artk-core-autogen directory exists
        if (pathStr.endsWith('artk-core-autogen')) return true;
        // index.js is missing
        if (pathStr.endsWith('index.js')) return false;
    ...
    it('should return false when installation is complete', () => {
      vi.mocked(fs.existsSync).mockImplementation((p) => {
        ...
        if (pathStr.includes('context.json')) return true;
        if (pathStr.includes('artk-core')) return true;
        if (pathStr.includes('artk-core-autogen')) return true;
        if (pathStr.includes('index.js')) return true;
  ...
  describe('createBackup', () => {
    ...
    it('should backup artk.config.yml if it exists', () => {
      ...
      expect(result.success).toBe(true);
      expect(fs.copyFileSync).toHaveBeenCalledWith(
        expect.stringContaining('artk.config.yml'),
        expect.stringContaining('artk.config.yml')
      );
    });
