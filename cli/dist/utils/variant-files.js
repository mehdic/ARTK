 ...
 */
export function getArtkVersion() {
 ...
 */
export function findArtkCorePath() {
    const possiblePaths = [
 ...
 */
export function getVariantDistPath(corePath, variant) {
 ...
 */
export function validateVariantBuildFiles(variant) {
 ...
 */
function getAutogenDistDirectory(variant) {
    switch (variant) {
        case 'modern-esm':
            return 'dist';
        case 'modern-cjs':
 ...
 */
export function copyVariantFiles(variant, targetPath) {
    ...
    try {
        ...
        if (!fs.existsSync(autogenDist)) {
            return {
                success: false,
                error: `Autogen package not found for variant '${variant}'. ` +
 ...
 */
export function generateVariantFeatures(variant) {
 ...
 */
export function writeVariantFeatures(vendorPath, variant) {
    const features = generateVariantFeatures(variant);
    fs.writeFileSync(path.join(vendorPath, 'variant-features.json'), JSON.stringify(features, null, 2), 'utf-8');
}
/**
 * Generate READONLY.md content.
 */
export function generateReadonlyMarker(variantDef, context) {
 ...
 */
export function writeReadonlyMarker(vendorPath, variantDef, context) {
 ...
 */
export function generateAiIgnore(variant) {
 ...
 */
export function writeAiIgnore(vendorPath, variant) {
 ...
 */
export function writeAllAiProtectionMarkers(vendorPath, variant, context) {
    const variantDef = getVariantDefinition(variant);
 ...
 */
export function checkVariantNodeCompatibility(variant, nodeVersion) {
    const variantDef = VARIANT_DEFINITIONS[variant];
 ...
 */
export function generateCopilotInstructions(variant) {
    const variantDef = VARIANT_DEFINITIONS[variant];
 ...
 */
export function writeCopilotInstructions(targetPath, variant) {
    const promptsDir = path.join(targetPath, '.github', 'prompts');
    fs.mkdirSync(promptsDir, { recursive: true });
    const content = generateCopilotInstructions(variant);
    fs.writeFileSync(path.join(promptsDir, 'artk.variant-info.prompt.md'), content, 'utf-8');
}
//# sourceMappingURL=variant-files.js.map