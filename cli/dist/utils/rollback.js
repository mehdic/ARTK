 ...
 */
export function rollback(targetPath, reason) {
 ...
 */
export function needsRollback(targetPath) {
    ...
    if (hasContext && (!hasVendorCore || !hasVendorAutogen)) {
        return {
            needed: true,
            reason: 'Partial installation detected: context exists but vendor directories are incomplete',
        };
    }
 ...
 */
export function createBackup(targetPath) {
    ...
    try {
        ...
        if (fs.existsSync(contextPath)) {
            fs.copyFileSync(contextPath, path.join(backupDir, 'context.json'));
        }
        // Backup artk.config.yml
        const configPath = path.join(targetPath, 'artk-e2e', 'artk.config.yml');
        if (fs.existsSync(configPath)) {
            fs.copyFileSync(configPath, path.join(backupDir, 'artk.config.yml'));
        }
 ...
 * Restore from backup.
 */
export function restoreFromBackup(targetPath, backupPath) {
    try {
