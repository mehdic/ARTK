 ...
 */
export function getVariantDefinition(id) {
 ...
 */
export function getVariantsForNodeVersion(nodeMajor) {
    return ALL_VARIANT_IDS.filter((id) => VARIANT_DEFINITIONS[id].nodeRange.includes(String(nodeMajor)));
}
/**
 ...
 */
export function getRecommendedVariant(nodeMajor, moduleSystem) {
    if (nodeMajor >= 18) {
    ...
    throw new Error(`Node.js ${nodeMajor} is not supported. ARTK requires Node.js 14 or higher.`);
}
/**
 * Check if a variant is compatible with a Node.js version.
 */
export function isVariantCompatible(variantId, nodeMajor) {
 ...
 */
export function getVariantHelpText() {
    const lines = ['Available variants:'];
    for (const variant of getAllVariants()) {
        const nodeRange = variant.nodeRange.join(', ');
        lines.push(`  ${variant.id.padEnd(12)} - ${variant.displayName} (Node ${nodeRange}, Playwright ${variant.playwrightVersion})`);
    }
    return lines.join('\n');
}
/**
...
//# sourceMappingURL=variant-definitions.js.map