...
import { detectEnvironmentChange, hasExistingInstallation, getNodeMajorVersion, } from '../utils/variant-detector.js';
import { getVariantDefinition, isVariantCompatible, MIN_NODE_VERSION, } from '../utils/variant-definitions.js';
...
export async function doctor(options) {
    ...
    if (envChange.changed) {
        ...
        if (envChange.currentVariant !== envChange.previousVariant) {
            recommendations.push(`Environment changed. Consider running \`artk upgrade\` to switch from '${envChange.previousVariant}' to '${envChange.currentVariant}'`);
        }
    }
    else {
        checks.push({
    ...
    if (!hasReadonly || !hasAiIgnore || !hasFeatures) {
        ...
        recommendations.push('Run `artk init --force` to regenerate AI protection markers');
    }
    else {
        checks.push({
            name: 'AI Protection Markers',
 ...
 */
export function printDoctorResults(result, verbose = false) {
    console.log('\nARTK Doctor Report\n');
    console.log('='.repeat(50));
    for (const check of result.checks) {
        const icon = check.status === 'pass' ? '✓' : check.status === 'warn' ? '⚠' : '✗';
        const color = check.status === 'pass' ? '32' : check.status === 'warn' ? '33' : '31';
