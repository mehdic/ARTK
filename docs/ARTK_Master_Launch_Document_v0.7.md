...
## Master Launch Document (Playbook + Detailed Plan) v0.7

P25-12-25
> **Contains:** Launch Playbook + Detailed Phased Plan (merged, versioned as v0.7)
...
- **/artk.init-playbook (MANDATORY)**  
  Scan the repository, detect stack and existing testing setup, ask only the *necessary* questions, then scaffold the ARTK root structure and configuration. Also generates the playbook guardrails and installs/updates the Journey system from **ARTK Core**.

### Discovery + foundation commands

- **/artk.discover-foundation (MANDATORY)**  
  Produce a consistent, semi-automated view of routes/features/auth/testability/risk and build the Playwright harness + ARTK foundation modules (env loader, auth setup project + storageState policy, navigation helpers, selector utilities, data harness scaffolding, module registry).

- **/artk.journey-propose (OPTIONAL)**  
  Automatically propose high-signal Journeys from discovery outputs. Writes `journeys/proposed/*` and updates backlog/index.

### Testability command

- **/artk.testid-audit (RECOMMENDED)**  
  Audit brittle selectors and missing test hooks, then produce a fix report and (optionally) apply safe data-* attributes.

### Journey authoring commands

- **/artk.journey-define (REQUIRED per journey)**  
...
- **/artk.journey-clarify (REQUIRED by default)**  
...
- **/artk.journey-implement (REQUIRED to automate)**  
...
- **/artk.journey-validate (REQUIRED)**  
...
- **/artk.journey-verify (REQUIRED for “implemented” status)**  
...
- **/artk.journey-maintain (RECOMMENDED ongoing, planned)**  
...
```mermaid
flowchart TD
  A["/artk.init-playbook MANDATORY"] --> B["/artk.discover-foundation MANDATORY"]
  B --> C["/artk.journey-propose OPTIONAL"]
  B --> D["/artk.journey-define REQUIRED per journey"]
  C --> D
  D --> E["/artk.journey-clarify REQUIRED default"]
  E --> F["/artk.testid-audit RECOMMENDED"]
  F --> G["/artk.journey-implement REQUIRED to automate"]
  G --> H["/artk.journey-validate REQUIRED"]
  H --> I["/artk.journey-verify REQUIRED for implemented status"]
  I -->|pass| J["Journey implemented BACKLOG checked"]
  I -->|fail| K["/artk.journey-verify heal OPTIONAL bounded"]
  K -->|pass| J
  K -->|still failing| L["Journey stays clarified or quarantined BACKLOG unchecked"]
  J --> M["/artk.journey-maintain RECOMMENDED ongoing (planned)"]
  L --> M
```
...
stateDiagram-v2
  direction LR
  [*] --> Proposed: OPT /artk.journey-propose or manual
  Proposed --> Defined: REQ /artk.journey-define
  Defined --> Clarified: REQ /artk.journey-clarify
  Clarified --> Implemented: REQ /artk.journey-implement then REQ /artk.journey-validate then REQ /artk.journey-verify
  Implemented --> Quarantined: REC /artk.journey-maintain
  Quarantined --> Clarified: REQ /artk.journey-clarify
  Quarantined --> Implemented: REQ /artk.journey-verify
  Proposed --> Deprecated: OPT /artk.journey-maintain
  Defined --> Deprecated: OPT /artk.journey-maintain
  Clarified --> Deprecated: OPT /artk.journey-maintain
  Implemented --> Deprecated: OPT /artk.journey-maintain
  Quarantined --> Deprecated: OPT /artk.journey-maintain
  Deprecated --> [*]
  Defined --> Implemented: OPT /artk.journey-implement allowNonClarified true
  Clarified --> Quarantined: OPT /artk.journey-maintain
```
...
-   Run /artk.init-playbook on one representative product
-   Run /artk.discover-foundation
-   Propose initial journeys (/artk.journey-propose)

### Phase 2

-   Define, clarify, and implement 5--10 journeys
-   Run /artk.testid-audit before implementation when selector debt appears

### Phase 3

-   Validate maintainability with /artk.journey-maintain (planned)
...
- Static gate passes: **/artk.journey-validate** reports no violations
- Runtime gate passes: **/artk.journey-verify** passes at least once in a supported environment
...
e2e/ (or artk/)
  docs/
    ...
    JOURNEY_PROPOSALS.md    # generated summary after /artk.journey-propose
  journeys/
...
- `/artk.journey-define`
- `/artk.journey-clarify` (medium by default; supports minimal/max)
- `/artk.testid-audit`
- `/artk.journey-implement`
- `/artk.journey-validate`
- `/artk.journey-verify`
- `/artk.journey-maintain` (planned)
...
## Phase 1 — Skeleton Bootstrap (/artk.init-playbook)
...
3. **Define `/artk.init-playbook` scanning checklist**
   `/artk.init-playbook` must scan for:
...
4. **Define `/artk.init-playbook` questions catalog (asked AFTER scan)**
   `/artk.init-playbook` asks targeted questions:
...
5. **Define `/artk.init-playbook` outputs**
   `/artk.init-playbook` must generate:
...
- Each repository uses an installer-style command (e.g., `/artk.init-playbook`) to **install or upgrade** the Journey system from Core and adapt it to the repo.
...
### Acceptance criteria
- `/artk.discover-foundation` output guides journey proposal and module planning.
- Known blockers are captured early, not discovered during test writing.

---

## Phase 5 — Automatic Journey Identification (/artk.journey-propose)
...
## Phase 6 — User-defined Journeys (/artk.journey-define) and Clarification (/artk.journey-clarify)
...
### Tasks
1. **Define `/artk.journey-define` input contract**
   The user provides:
   - goal
   ...
   **Discovery-aware behavior (recommended):**
   - If `/artk.discover-foundation` outputs exist, prefill likely scope/routes/modules, and ask only for confirmation when ambiguous.

2. **Define `/artk.journey-clarify` (medium by default)**
...
### Acceptance criteria
- A clarified journey is sufficient for `/artk.journey-implement` to implement without guessing key details.


---

## Phase 7 — Foundation Build (part of /artk.discover-foundation) + Playwright Baseline Harness
...
## Phase 8 — Implement MVP Journeys (/artk.journey-implement) + Validation (/artk.journey-validate) + Verification (/artk.journey-verify)
...
- `/artk.journey-validate` passes, **and**
- `/artk.journey-verify` passes at least once in a supported environment.
...
2. **Define `/artk.journey-implement` behavior (generator)**
   For each selected Journey:
   - confirm Journey is `clarified` (or apply policy: allow skeleton only when explicitly permitted)
   - ensure foundation modules exist (require `/artk.discover-foundation` if missing)
...
3. **Add `/artk.journey-validate` static gate (mandatory)**
...
4. **Add `/artk.journey-verify` runtime gate (mandatory for implemented status)**
...
5. **Update Journey status correctly**
   ...
   - If unstable: quarantine via `/artk.journey-maintain` (do not pretend it’s implemented).
...
## Phase 9 — Maintenance Capability (/artk.journey-maintain, planned)
...
1. **Standard onboarding workflow**
   ...
   - `/artk.journey-propose`
   - select MVP journeys
   - `/artk.testid-audit` (as needed before implementation)
   - implement MVP journeys (`/artk.journey-define`, `/artk.journey-clarify`, `/artk.journey-implement`)
...
- `.github/prompts/artk.testid-audit.prompt.md`
- `.github/prompts/artk.journey-implement.prompt.md`
- `.github/prompts/artk.journey-validate.prompt.md`
- `.github/prompts/artk.journey-verify.prompt.md`
- `.github/prompts/artk.journey-maintain.prompt.md` (planned)
...
- **Suite rot without maintenance**
  - Mitigation: `/artk.journey-maintain` from day one; ownership rules.
